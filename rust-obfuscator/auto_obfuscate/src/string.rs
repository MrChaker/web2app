...
use quote::quote;
use syn::{
    parse_file, parse_quote, parse_str, visit::Visit, visit_mut::VisitMut, Expr, ExprLit, File,
    Lit, LitStr, Local,
};
...
impl StringObfuscator {
                ...
                (match token {
                    TokenTree::Literal(lit) => {
                        ...
                        if let Ok(lit_str) = parse_str::<LitStr>(&lit_str) {
                            let macro_call: TokenStream = quote! {
    ...
    pub fn obfuscate_strings(&mut self, code: &str) -> String {
        ...
        let strings_to_encrypt =
            (((self.percentage as f32) / 100.0) * (total_strings as f32)).ceil() as usize;
