...
mod rename;
mod string;
use crate::obfuscate::{Config, Obfuscator};
use clap::{App, Arg};
use std::fs;
use std::path::Path;
...
fn main() {
    ...
    let matches = App::new("Rust Automatic Obfuscator")
        ...
        .arg(
            Arg::with_name("no_string")
                .long("no_string")
                .help("Disable string obfuscation"),
        )
        .arg(
            Arg::with_name("no_flow")
                .long("no_flow")
    ...
    if let Some(percentage) = matches.value_of("p") {
        config.string_config.percentage = match percentage.parse() {
            ...
            _ => {
                eprintln!(
                    "-p: expected integer between 0 and 100, got: `{}`",
                    percentage
                );
...
fn write_obfuscated_code(original_path: &Path, obfuscated_code: &str) {
    ...
    fs::write(original_path, obfuscated_code).expect("Failed to write obfuscated code");
}
